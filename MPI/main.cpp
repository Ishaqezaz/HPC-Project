...
int main(int argc, char** argv) {
    ...
    // meshgrid creation (done by all processes)
    int N = 180;
    ...
    // each process handles a subset of angles
    int anglesPerProc = Nangles / size;
    int startAngle = rank * anglesPerProc;
    int endAngle = (rank == size - 1) ? Nangles : startAngle + anglesPerProc;

    for (int angleIndex = startAngle; angleIndex < endAngle; angleIndex++) {
        double angle =  M_PI / 2 * angleIndex / Nangles;

        // rotation on the meshgrid
        ...
        std::string imageName = "images/volumerender" + std::to_string(rank) + "_" + std::to_string(angleIndex) + ".png";
